...
- Unauthorized users face bearer/session authentication, high-entropy hashed tokens, expiring opaque sessions, CSRF and same-origin checks on browser mutations, exact Host validation, request bounds, per-source login limits, and scan-creation rates. Localhost is the default bind; non-loopback exposure requires separate bind, Host, and cookie-safety decisions.

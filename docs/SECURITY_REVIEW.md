...
- Authentication: protected API resources reject anonymous users; browser mutations require CSRF plus same-origin/fetch-metadata checks; session refresh is POST-only; sessions expire; rotation uses a staged owner-only file replacement and revokes tokens/sessions; login limits are per remote source and denials are audited; JSON bodies are bounded and strict. Bearer/session audit actors include their durable IDs.
- Control-plane exposure: exact Host allowlists reject DNS-rebinding Host values. Non-loopback binds require explicit hosts and secure cookies unless a second unsafe cookie override is set for a loopback-published container topology. Existing data directories, databases, and token files have owner-only modes repaired.
...
- The application does not terminate TLS. Non-loopback binds require secure cookies by default, exact external Host values, and a maintained TLS reverse proxy; the separate insecure-cookie override is reserved for explicitly loopback-published container topologies.
- Login limits use the direct socket source because proxy headers are intentionally untrusted; multiple operators behind one NAT share a bucket, and public deployments should also rate-limit at the authenticated reverse proxy.
